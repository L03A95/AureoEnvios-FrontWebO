...
export default function Register() {
  ...
  const submitUser = async (e: React.MouseEvent) => {
    e.preventDefault();
    try {
      await sendUserInfoToAPI(user, 'fisica').then(() => { setVerification(true) });
    } catch (error) {
  ...
  return (
    <div className="form_wrapper">
      {verification ? <Verification email={user.credenciales.email}/> : (
        <form className="login_form">
          ...
          <input
            ...
            onChange={handleChange}
            name="username"
            value={user.credenciales.username}
          />
          <input
            type="password"
            ...
            onChange={handleChange}
            name="password"
            value={user.credenciales.password}
          />
          <div>
            <select
              value={user.credenciales.codigoDeLlamada}
              onChange={handleChange}
              name={"codigoDeLlamada"}
              className="login_input_codigo_llamada"
            >
            ...
            <input
              ...
              onChange={handleChange}
              name="celular"
              value={user.credenciales.celular}
            />
          </div>
